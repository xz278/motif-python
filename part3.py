if __name__ == '__main__':
	...
	# eureka = pd.read_csv('temp1.csv', index_col = 0, usecols=['time', 'longitude', 'latitude','geo_hash'])
	l = pd.Series.from_csv('temp2.csv', index_col = 0)
	x = pd.read_csv('temp3.csv',usecols = [1,2]).as_matrix().tolist()
	# print(x)
	...
	pd.DataFrame(vectorized_haversine2(geohash_clusters)).to_csv(newpath + '/' + uid + '-geohash-cluster-dist.csv')
	# motif2.plot_cluster(x = eureka.ix[:,1:3].as_matrix(), y = geohash_labels, outputfilename = newpath + '/' + uid + '-geohash-cluster-plot.png')

	print('part3 ends.')

	timeseries = eureka.index
	clusterseries = eureka.ix[:,3].as_matrix()
	tempd = {}
	n = len(timeseries)
	night_time = [3,4,5]
	for i in range(n):
	...
	home_cluster = []
	for item in tempd.keys():
		idx = q.index(item)
		home_cluster.append(x[idx])

	start_point = home_cluster[0]
	# print start_point
	cluster_map = folium.Map(location = [start_point[0],start_point[1]])
	folium.LatLngPopup().add_to(cluster_map)
	...
	for c in range(len(tempd)):
		centroid = [home_cluster[c][0],home_cluster[c][1]]
		cluster_size = list(tempd.values())[c]
		folium.Marker(location = centroid,popup = str(cluster_size), icon = folium.Icon(color = 'red',icon = 'info-sign')).add_to(cluster_map)

	cluster_map.save(newpath + '/' + uid + '-home-cluster.csv')