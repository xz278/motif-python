...
config = 'config.csv' # paras for dbscan	
...
l = pd.Series.from_csv('temp2.csv')
...
l = pd.Series.from_csv('temp3.csv', index_col = [0,1])
...
pd.DataFrame(vectorized_haversine2(geo_clusters)).to_csv(newpath + '/' + uid + '-geohash-cluster-dist.csv')
...
cluster_map.save(newpath + '/' + uid + '-home-cluster.html')